...
import { getFeaturedTools, UNIT_TOOL } from "@/config/tools";
import {
  UNIT_TOOL_FAQ,
  UNIT_TOOL_HOW_TO,
  UNIT_TOOL_USE_CASES,
} from "@/lib/unit-converter-content";
import {
  breadcrumbSchema,
  faqPageSchema,
...
export const metadata = buildMetadata({
  ...
  description:
    "Convert length, weight, temperature, volume, area, and speed units instantly in your browser.",
  path: UNIT_TOOL.href,
  keywords: [...UNIT_TOOL.keywords],
});

/**
 * Free Unit Converter — third tool in the Googie Tools platform.
 */
export default function UnitConverterPage() {
  const relatedTools = getFeaturedTools(6).filter(
    (tool) => tool.slug !== UNIT_TOOL.slug,
  );
  ...
  return (
    <>
      ...
      />
      <JsonLd data={faqPageSchema(UNIT_TOOL_FAQ)} />
      <JsonLd
        data={breadcrumbSchema([
          { name: "Home", path: "/" },
          { name: "Tools", path: "/#tools" },
          { name: "Unit Converter", path: UNIT_TOOL.href },
        ])}
      />

      <ToolPageHeader
        title="Free Unit Converter"
        description="Convert common units of length, weight, temperature, volume, area, and speed directly in your browser."
      ...
      <section className="pt-2 pb-2 sm:pt-2.5" aria-labelledby="generator-heading">
        <Container>
          ...
          <TrustNote className="mt-5">
            <p>
              Conversions are calculated on your device and are not uploaded to
              our servers.
            </p>
          </TrustNote>
        </Container>
        ...
        <Container>
          <div className="mb-9 max-w-2xl sm:mb-11">
            <SectionTitle
              id="how-to-heading"
              sparkle={false}
              description="Three quick steps from category to a ready result."
            >
              How to use it
            </SectionTitle>
          </div>
          <ol className="grid list-none gap-8 p-0 sm:grid-cols-3">
            {UNIT_TOOL_HOW_TO.map((step, index) => (
              <li key={step.title}>
                <article>
        ...
        <Container>
          <div className="mb-9 max-w-2xl sm:mb-11">
            <SectionTitle
              ...
              sparkle={false}
              description="Practical ways people use this free unit converter."
            >
              Common use cases
            </SectionTitle>
          </div>
          <ul className="grid list-none gap-8 p-0 sm:grid-cols-3">
            {UNIT_TOOL_USE_CASES.map((item) => (
              <li key={item.title}>
                <article>
        ...
        <Container>
          <div className="mb-9 max-w-2xl sm:mb-11">
            <SectionTitle
              id="faq-heading"
              sparkle={false}
              description="Quick answers about privacy, accuracy, and how conversion works."
            >
              FAQ
            </SectionTitle>
          </div>
          <div className="mx-auto flex max-w-2xl flex-col gap-3">
            {UNIT_TOOL_FAQ.map((item) => (
              <AccordionItem
                key={item.question}
